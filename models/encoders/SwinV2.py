...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01, device=self.logit_scale.device, dtype=self.logit_scale.dtype))).exp()
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 4, 4], num_heads=[3, 6, 12, 24],
    ...
    def forward(self, x, cnn_features):
        x, intermediate_features = self.forward_features(x, cnn_features)
        return x, intermediate_features
